...
func (suite *TripSuite) SetupTest() {
...
func (suite *TripSuite) TearDownTest() {
...
func TestTripSuite(t *testing.T) {
	suite.Run(t, new(TripSuite))
}

func (suite *TripSuite) TestTripListValidTripsSuccessful() {
	//Arrange
	...
	suite.tripService = service.NewTripService(suite.repository, med)
	...
	//Act
	trips, err := suite.tripService.ListValidTrips(ctx, 25, 0)

	//Assert
	suite.Nil(err)
	suite.Equal(1, len(trips))
}
