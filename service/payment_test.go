...
func (suite *PaymentSuite) TestPaymentGetPaymentByReservationSuccessful() {
	//Arrange
	med := media.NewStoreMock()
	med.On("Put", mock.AnythingOfType("string"), mock.AnythingOfType("media.Permission"), mock.AnythingOfTypeArgument("*bytes.Reader")).Return("id", nil)
	...
	//Act
	record, err := suite.paymentService.GetPaymentByReservation(ctx, 1)

	//Assert
	suite.Nil(err)
	suite.Equal(payment.ID, record.ID)
}
