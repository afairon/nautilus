...
func (suite *LiveaboardSuite) SetupTest() {
	var err error
...
func (suite *LiveaboardSuite) TearDownTest() {
	db, _ := suite.db.DB()
	db.Close()
}

func TestLiveaboardSuite(t *testing.T) {
	suite.Run(t, new(LiveaboardSuite))
}

func (suite *LiveaboardSuite) TestLiveaboardGetLiveaboardSuccessful() {
	//Arrange
	...
	suite.liveaboardService = service.NewLiveaboardService(suite.repository, med)
	...
	//Act
	record, err := suite.liveaboardService.GetLiveaboard(ctx, 1)

	//Assert
	suite.Nil(err)
	suite.Equal(liveaboard.Name, record.Name)
}
